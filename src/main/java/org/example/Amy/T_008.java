 ...
 * @Description: 背包问题（尝试模型） (暴力递归+动态规划)
...
public class T_008 {
     ...
     * @param values    货物价值
     * @param index     货物索引
     * @param alreadyW  已经选择的总价值
     * @param bag       背包重量
     * @Return -1:认为没有方案
     */
    public static int function(int[] weights, int[] values, int index, int alreadyW, int bag){
        if (alreadyW > bag){
            return -1;
        }
        ...
        int no = function(weights,values,index+1,alreadyW,bag);
        int yes = function(weights,values,index+1,alreadyW+weights[index],bag);

        if (yes != -1){
            return Math.max(no, yes+values[index]);
        }
        return no;
    }
    ...
    public static int function2(int[] w, int[] v, int index, int rest){
        ...
        if (yes != -1){
            return Math.max(no, yes+v[index]);
        }
        return no;
    }
